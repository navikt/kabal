...
/**
 * How long to wait for the Kafka stream to become readable during init before
 * continuing. Startup does not block on Kafka — the sync loop handles recovery
 * if Kafka is unavailable.
 */
const KAFKA_READY_TIMEOUT_MS = 10_000;
...
class SmartDocumentWriteAccess {
  ...
  #connectKafka = async (): Promise<void> => {
    ...
    try {
      ...
      // Wait for the stream to become readable, but do not block startup forever.
      await new Promise<void>((resolve) => {
        const timeout = setTimeout(() => {
          cleanup();
          resolve();
        }, KAFKA_READY_TIMEOUT_MS);
        ...
        const cleanup = () => {
          initialStream.removeListener('readable', onReadable);
          clearTimeout(timeout);
        };

        initialStream.once('readable', onReadable);
      });
    } catch (error) {
      log.error({
      ...
      Array.from(documentIds, async (documentId) => {
        try {
          ...
          if (error instanceof DocumentAccessNotFoundError) {
            log.info({
              msg: 'Document deleted (404) while polling, removing from access map',
              trace_id,
  ...
  hasAccess = async (
    ...
    if (allowApiFetching) {
      try {
        ...
        if (error instanceof DocumentAccessNotFoundError) {
          log.info({
            msg: 'Document deleted (404) on access check, removing from access map',
            trace_id: this.#lifecycle_trace_id,
            data: { document_id: documentId },
          });
