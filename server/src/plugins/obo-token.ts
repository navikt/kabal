...
import { closeConnection } from '@/plugins/crdt/close-event';
    ...
    async () => {
      try {
        ...
        if (!oboAccessToken.ok) {
          log.warn({
            msg: `Failed to get OBO token for audience: ${audience}: ${oboAccessToken.error.name} - ${oboAccessToken.error.message}`,
            ...logParams,
            data: { ...logParams.data, stack: oboAccessToken.error.stack },
          });
...
export const getAuthorizationHeader = async (context: ConnectionContext): Promise<string> => {
  const { behandlingId, dokumentId, tab_id, client_version, accessToken } = context;
  ...
  if (accessToken === undefined) {
    ...
    throw closeConnection(context, 'MISSING_AUTHORIZATION', 4401);
  }
  ...
  if (oboAccessToken === undefined) {
    log.error({
      msg: 'Failed to get OBO token. Closing connection.',
      tab_id,
      client_version,
      data: { behandlingId, dokumentId },
    });

    throw closeConnection(context, 'MISSING_OBO_TOKEN', 4401);
  }

  return `Bearer ${oboAccessToken}`;
};
