...
import { getOboTokenMiddleware } from '@app/middleware/obo-token';
...
export const setupDocumentRoutes = (app: FastifyInstance) => {
  ...
  if (isDeployed) {
    ['/arkivert-dokument/*', '/nytt-dokument/*', '/vedleggsoversikt/*', '/kombinert-dokument/*'].forEach((route) => {
      app.register(route, getOboTokenMiddleware('kabal-api', route));
    });
  }
