      ...
      COLUMNS.map((col) => (
        ...
        >
          <BoxNew
            ...
            className={'h-6 w-6 cursor-pointer bg-transparent hover:bg-surface-action-subtle-hover'}
          />
        </Tooltip>
      )),
    )}
...
const BackgroundGrid = memo(() => (
  <HGrid gap="05" width="max-content" overflow="hidden" className="z-0" columns={MAX_COLUMNS_COUNT}>
    {ROWS.map((row) =>
      COLUMNS.map((col) => <BoxNew key={`${row}-${col}`} background="accent-soft" className="h-6 w-6" />),
    )}
  </HGrid>
));
