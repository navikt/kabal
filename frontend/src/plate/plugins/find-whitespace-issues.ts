...
/** Match a string that is entirely whitespace. */
const WHITESPACE_ONLY = /^\p{Zs}+$/u;
/** Match a leading punctuation character (same set as SPACE_BEFORE_PUNCTUATION lookahead). */
const STARTS_WITH_PUNCTUATION = /^[.,:;!?)\]»]/u;
...
export interface FindWhitespaceIssuesOptions {
  ...
  /** When true, even a single trailing space at the end of the element is flagged as TrailingWhitespace. Spaces immediately before forced newlines are unaffected. */
...
export const findWhitespaceIssues = (
  ...
  findWhitespaceOnlyInlineIssues(element, elementPath, issues);
...
const findTrailingIssues = (
  ...
  const focus = toPoint(segments, end, elementPath);

  coveredRanges.push({ start: match.index, end });
...
/** Detect cross-boundary issues: inline element trailing space + text leading space or punctuation. */
const findCrossBoundaryForwardIssues = (
  ...
  // Case 1: text starts with punctuation → space before punctuation (delete space inside inline element).
  if (STARTS_WITH_PUNCTUATION.test(text)) {
    issues.push({
      type: WhitespaceIssueType.SpaceBeforePunctuation,
      anchor: inlineTrailingSpaceRange.anchor,
      focus: toPoint(segments, 0, elementPath),
      deleteRanges: [inlineTrailingSpaceRange],
    });

    return;
  }

  // Case 2: text starts with space → double space across boundary.
  const match = text.match(LEADING_WHITESPACE);
  ...
  // Delete only the last trailing space from the inline element (extras handled by inline sub-pass).
  const deleteRanges: TRange[] = [
    {
      anchor: { path: inlineTrailingSpaceRange.focus.path, offset: inlineTrailingSpaceRange.focus.offset - 1 },
      focus: inlineTrailingSpaceRange.focus,
    },
  ];
...
/** Flag inline children whose entire text content is whitespace. */
const findWhitespaceOnlyInlineIssues = (element: TElement, elementPath: Path, issues: WhitespaceIssue[]): void => {
  for (const [index, child] of element.children.entries()) {
    if (!ElementApi.isElement(child)) {
      continue;
    }

    const range = findWhitespaceOnlyRange(child, [...elementPath, index]);
...
/** If the element has a single text child that is entirely whitespace, return its full range. */
const findWhitespaceOnlyRange = (element: TElement, basePath: Path): TRange | undefined => {
  const firstChild = element.children[0];

  if (element.children.length !== 1 || !TextApi.isText(firstChild) || !WHITESPACE_ONLY.test(firstChild.text)) {
    return undefined;
  }

  const path = [...basePath, 0];

  return { anchor: { path, offset: 0 }, focus: { path, offset: firstChild.text.length } };
};

/** Find the range covering all trailing whitespace in an element's deepest last text node. */
const findTrailingSpaceRange = (element: TElement, basePath: Path): TRange | undefined => {
  ...
  if (TextApi.isText(lastChild)) {
    const match = lastChild.text.match(ALL_TRAILING_WHITESPACE);
    ...
    return { anchor: { path, offset: match.index }, focus: { path, offset: lastChild.text.length } };
  }
