      ...
      onQueryStarted: async ({ oppgaveId, hjemler }, { queryFulfilled }) => {
        ...
        try {
          ...
          toast({
            ...
            message: hjemler.length === 0 ? 'Innsendingshjemler fjernet' : 'Innsendingshjemler endret',
          });
        } catch {
          toast({ type: ToastType.ERROR, message: 'Feil ved endring av innsendingshjemler' });
          undo();
        }
      },
    }),
