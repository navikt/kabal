...
import { loadKodeverk } from '../tilstand/moduler/kodeverk';
...
export default function Oppsett({ visMeny, backLink, customClass, contentClass, children }: LayoutType) {
  ...
  useEffect(() => {
    ...
    if (R.empty(kodeverk)) {
      dispatch(loadKodeverk());
    }
  }, [dispatch, loadKodeverk, hentFeatureToggleHandling]);
  ...
  const enhetensOppgaverEnabled = featureToggles.features.find((f) => f?.navn === 'klage.enhetensoppgaver');
          ...
          return (
            ...
            >
              <Alertstripe type={visToaster.type}>
                <span>{typeof toastBeskjed === 'string' ? toastBeskjed : JSON.stringify(toastBeskjed)}</span>
              </Alertstripe>
            </div>
          );
        })()}
