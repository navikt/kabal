...
const getLines = (initialOffset: number) => {
  ...
  for (const { key, width, height } of LINES) {
    lines.push(<Line key={key} x={PDF_PADDING_X} y={lineOffset} width={MAX_CONTENT_WIDTH * width} height={height} />);
...
const PdfContent = ({ title, customScale = 100, scaleMode }: PdfContentProps) => {
  ...
  const pageCount = Math.ceil(screen.height / (PDF_PAGE_HEIGHT / (100 / MIN)));
  ...
  return (
    ...
    >
      ...
      {/* Address */}
      <Line x={PDF_PADDING_X} y={PDF_PADDING_Y} width={MAX_CONTENT_WIDTH * 0.1} height={LINE_FONT_SIZE} />
      <Line
        x={PDF_PADDING_X}
...
const Line = ({ x, y, width, height }: LineProps) => (
