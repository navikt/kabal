...
import {
  useOppgaveTableFerdigstiltFrom,
  useOppgaveTableFerdigstiltTo,
  useOppgaveTableVarsletFristFrom,
  useOppgaveTableVarsletFristTo,
...
const Frist = ({ children, tableKey, ...props }: TableFilterHeadersProps & { children: string }) => {
  const [fristFrom, setFristFrom] = useOppgaveTableFerdigstiltFrom(tableKey);
  const [fristTo, setFristTo] = useOppgaveTableFerdigstiltTo(tableKey);
...
const VarsletFrist = ({ children, tableKey, ...props }: TableFilterHeadersProps & { children: string }) => {
  ...
  const [varsletFristFrom, setVarsletFristFrom] = useOppgaveTableVarsletFristFrom(tableKey);
  const [varsletFristTo, setVarsletFristTo] = useOppgaveTableVarsletFristTo(tableKey);

  return (
    <DateColumnHeader
      {...props}
      from={varsletFristFrom}
      to={varsletFristTo}
      setFrom={setVarsletFristFrom}
      setTo={setVarsletFristTo}
...
const Finished = ({ children, tableKey, ...props }: TableFilterHeadersProps & { children: string }) => {
  const [ferdigstiltFrom, setFerdigstiltFrom] = useOppgaveTableFerdigstiltFrom(tableKey);
  const [ferdigstiltTo, setFerdigstiltTo] = useOppgaveTableFerdigstiltTo(tableKey);
  ...
  return (
    <DateColumnHeader
      ...
      to={ferdigstiltTo}
      setFrom={setFerdigstiltFrom}
      setTo={setFerdigstiltTo}
...
const Returnert = ({ children, tableKey, ...props }: TableFilterHeadersProps & { children: string }) => {
  const [returnertFrom, setReturnertFrom] = useOppgaveTableFerdigstiltFrom(tableKey);
  const [returnertTo, setReturnertTo] = useOppgaveTableFerdigstiltTo(tableKey);
  ...
  return (
    <DateColumnHeader
      ...
      from={returnertFrom}
      to={returnertTo}
      setFrom={setReturnertFrom}
      setTo={setReturnertTo}
