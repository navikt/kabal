...
const Selection = ({ gosysOppgave, selected, onSelect, isSelecting }: SelectionProps) => {
  if (selected) {
    return (
      <Button size="small" variant="tertiary" loading={isSelecting} title="Valgt" aria-label="Valgt" as="span">
        <CheckmarkCircleFillIconColored aria-hidden fontSize={20} />
      </Button>
    );
  }
...
const AlreadyUsedIconContainer = styled.div`
  display: flex;
  justify-content: center;
  align-items: center;
`;

const Enhet = ({ enhet }: { enhet: string }) => {
...
const Employee = ({ employee }: { employee: INavEmployee | null }) => {
  if (employee === null) {
    return null;
  }
