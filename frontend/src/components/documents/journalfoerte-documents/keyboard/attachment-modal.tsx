...
export const AttachmentModal = ({ ref, focusedDocument, focusedVedlegg }: Props) => {
  ...
  const onKeyDown = (event: React.KeyboardEvent) => {
    ...
    switch (event.key) {
      case 'ArrowUp':
        setFocused(mod ? 0 : (prev) => decrement(prev, 1, 0, options.length - 1));
        break;
      case 'ArrowDown':
        setFocused(mod ? options.length - 1 : (prev) => increment(prev, 1, options.length, 0));
        break;
      case 'PageUp':
        setFocused(mod ? 0 : (prev) => decrement(prev, 10, 0, options.length - 1));
        break;
      case 'PageDown':
        setFocused(mod ? options.length - 1 : (prev) => increment(prev, 10, options.length, 0));
        break;
      case 'Home':
        setFocused(0);
        break;
