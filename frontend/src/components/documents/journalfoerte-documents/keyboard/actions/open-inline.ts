...
export const useOpenInline = (filteredDocuments: IArkivertDocument[]) => {
  const { value: pdfViewed, setArchivedDocuments, remove } = useDocumentsPdfViewed();
  ...
  return useCallback(async () => {
    ...
    if (selectedDocuments.size > 0 && isSelected(focusedIndex)) {
      ...
      const archivedDocuments = pdfViewed.archivedDocuments;
    ...
    const isOpen =
      ...
      pdfViewed.archivedDocuments[0]?.dokumentInfoId === dokumentInfoId &&
      pdfViewed.archivedDocuments[0]?.journalpostId === journalpostId;
