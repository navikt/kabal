...
describe('deduplicateRanges', () => {
  it('should return empty array for empty input', () => {
    const ranges: SelectionRange[] = [];
    const deduplicated = deduplicateRanges(ranges);
    expect(deduplicated).toEqual([]);
  });

  it('should return unchanged range for single range', () => {
    const ranges = [r(0, 1)];
    const deduplicated = deduplicateRanges(ranges);
    expect(deduplicated).toEqual(ranges);
  });

  it('should return a single range for duplicate ranges', () => {
    const ranges = [r(0, 1), r(0, 1)];
    const deduplicated = deduplicateRanges(ranges);
    expect(deduplicated).toEqual([r(0, 1)]);
  });

  it('should return unchanged ranges for non-duplicate ranges', () => {
    const ranges = [r(0, 1), r(2, 3)];
    const deduplicated = deduplicateRanges(ranges);
    expect(deduplicated).toEqual(ranges);
  });

  it('should return ranges in same order as input (last duplicate range wins)', () => {
    const ranges = [r(0, 1), r(2, 3), r(0, 1)];
    const deduplicated = deduplicateRanges(ranges);
    expect(deduplicated).toEqual([r(2, 3), r(0, 1)]);
  });
});
