...
export const SharedDocumentTitle = (props: Props) => {
  ...
  const { value, setNewDocument, setArchivedDocuments, setVedleggsoversikt } = useDocumentsPdfViewed();
  ...
  const isInlineOpen = useMemo(() => {
    if (type === DocumentTypeEnum.JOURNALFOERT) {
      return value.archivedDocuments?.some((d) => d.dokumentInfoId === dokumentInfoId) ?? false;
    }
