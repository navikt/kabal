...
const Tabbed = ({ documents }: TabbedProps) => {
  ...
  const accessibleDocuments = documents.filter((document) => {
    const { read } =
      document.parentId === null
        ...
        : getAttachmentAccess(document, getParentDocument(document.parentId));

    return read;
  });

  const [firstDocument] = accessibleDocuments;
  ...
  const activeEditorId =
    editorId !== undefined && accessibleDocuments.some(({ id }) => id === editorId) ? editorId : defaultEditorId;
  ...
  if (accessibleDocuments.length === 0 && !canCreateMainDocuments) {
    return (
      <PanelContainer>
    ...
    <PanelContainer>
      <Tabs className="flex h-full flex-col overflow-hidden" value={activeEditorId} onChange={setEditorId} size="small">
        <Tabs.List className="whitespace-nowrap" style={{ maxWidth: getMaxWidth(showGodeFormuleringer, showHistory) }}>
          {accessibleDocuments.map(({ id, tittel }) => (
