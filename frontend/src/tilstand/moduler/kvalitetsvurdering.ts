...
export const kvalitetsvurderingSlice = createSlice({
  ...
  reducers: {
    ...
    HENT: (state) => ({ ...state, hentet: false, laster: true }),
    SETT_VERSJON: (state, action: PayloadAction<number>) => ({
      ...state,
      klagebehandlingVersjon: action.payload,
    }),
...
export const lagreKvalitetsvurdering = createAction<Partial<IKvalitetsvurdering>>('kvalitetsvurdering/LAGRE');
export const hentetHandling = createAction<IKvalitetsvurdering>('kvalitetsvurdering/HENTET');
export const lagretHandling = createAction<Partial<IKvalitetsvurdering>>('kvalitetsvurdering/LAGRET');
